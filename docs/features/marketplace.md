...
An admin can **unlist** any published item as a kill-switch (**Admin** console). An unlisted
item disappears from the Marketplace and from new adoptions, and is dropped from adopters' live
runtime — an unlisted workflow won't fire even if an adopter's schedule still ticks. Unlisting is
**reversible**: re-listing resumes everything with no re-wiring.
