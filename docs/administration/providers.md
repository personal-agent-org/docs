...
A header toggle switches the layout between **By provider** (the default) and
**By model**. The model view pivots the same data so each model id appears once
with every provider that offers it underneath - useful when the same model is
sold by several providers (e.g. a frontier model on a direct account and a
proxy). The per-model controls (enable, tags, quality, pricing) are identical in
both views.
...
active`); a keyless provider instead shows `Keyless - ready to use`.
